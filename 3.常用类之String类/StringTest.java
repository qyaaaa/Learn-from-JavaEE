...
public class StringTest {
    ...
    String：字符串，使用一对""引起来表示。
     ...
     3.String内部定义了 final char【】 vaule用于存储字符串数据
     4.String：代表不可变的字符序列。简称：不可变性
            体现：1.当对字符串重新赋值时，需要重写指定内存区域赋值，不能使用原有的value进行赋值
                 2.当对现有的字符串进行连接操作时，也需要重新指定内存区域赋值，不能使用原有的value赋值
                 3.当调用String的replace（）方法修改指定的字符或字符串时，也需要指定内存区域赋值，不能使用原有的value赋值
     ...
     *          两个。一个是堆空间中new的，另一个是char【】对应的常量池中的数据abc
     */
    @Test
    public void test2(){
        //此时的s1.s2声明在方法区的字符串常量池中
        String s1 = "java";
        String s2 = "java";
     ...
     * 2.只要其中有一个是变量，结果就在堆中
     * 3.只要拼接的结果调用intern（）方法，返回值就在常量池中
     */
    @Test
    public void test3(){
        String s1 = "javaee";
        ...
        String s8 = s5.intern();//返回值得到的s8使用的常量池中已经存在的值
        System.out.println(s3 == s8);//true

        final String s9 = "javaee";//s9:常量
